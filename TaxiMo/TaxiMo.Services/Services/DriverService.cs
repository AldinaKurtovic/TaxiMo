        ...
        public async Task<bool> DeleteAsync(int id)
        {
            try
            {
                ...
                bool hasRequestedRides = await _context.Rides
                    .AnyAsync(r => r.DriverId == id && r.Status == "requested");

                if (hasRequestedRides)
                {
                    throw new InvalidOperationException("Driver cannot be deleted because they have requested rides. Please complete or cancel these rides first.");
                }

                // Delete all rides (completed and active) for this driver (and related entities) before deleting the driver
                var allRides = await _context.Rides
                    .Where(r => r.DriverId == id && (r.Status == "completed" || r.Status == "active"))
                    .ToListAsync();
