...
namespace TaxiMo.Services.Database
{
    public static class DataSeed
    {
        ...
        private static async Task UpsertRidesAsync(TaxiMoDbContext context, DateTime baseDate)
        {
            ...
            // 1. Main driver ("driver") - najviše completed vožnji (20 vožnji)
            var mainDriverRides = 20;
            var mainDriverRiders = allUsers.Take(Math.Min(mainDriverRides, allUsers.Count)).ToList();
            for (int i = 0; i < mainDriverRides; i++)
            {
                var rider = mainDriverRiders[i % mainDriverRiders.Count];
                ...
                var requestedAt = baseDate.AddDays(-(mainDriverRides - i)).AddHours(-i * 2);
            ...
            // 2. Mobile user - najviše completed vožnji sa različitim driverima (15 vožnji)
            var mobileRides = 15;
            var availableDriversForMobile = otherDrivers.Take(Math.Min(mobileRides, otherDrivers.Count)).ToList();
            for (int i = 0; i < mobileRides; i++)
            {
                ...
                var pickupLoc = locations[i % locations.Count];
                var dropoffLoc = locations[(i + 3) % locations.Count];
                
                if (pickupLoc.LocationId == dropoffLoc.LocationId)
                {
                    dropoffLoc = locations[(i + 5) % locations.Count];
                }

                var requestedAt = baseDate.AddDays(-(mobileRides - i + 5)).AddHours(-i * 3);
            ...
            // 3. Other drivers - po jedna requested vožnja
            foreach (var driver in otherDrivers)
            {
                var driverVehicle = vehicleByDriver.GetValueOrDefault(driver.DriverId);
                if (driverVehicle == null) continue;

                var rider = allUsers[driver.DriverId % allUsers.Count];
                var pickupLoc = locations[driver.DriverId % locations.Count];
                var dropoffLoc = locations[(driver.DriverId + 1) % locations.Count];
                
                if (pickupLoc.LocationId == dropoffLoc.LocationId)
                {
                    dropoffLoc = locations[(driver.DriverId + 2) % locations.Count];
                }

                var requestedAt = baseDate.AddDays(-(driver.DriverId % 10)).AddHours(-(driver.DriverId % 24));
                var fare = 7.0m + (driver.DriverId % 10) * 1.2m;
            ...
            // 4. Mirza Begic - jedna active vožnja
            var mirzaBegic = await context.Drivers.FirstOrDefaultAsync(d => d.Username == "driver.mirza");
            if (mirzaBegic != null)
            {
                var mirzaVehicle = vehicleByDriver.GetValueOrDefault(mirzaBegic.DriverId);
                if (mirzaVehicle != null && allUsers.Count > 0 && locations.Count >= 2)
                {
                    var rider = allUsers[0]; // Prvi korisnik
                    var pickupLoc = locations[0]; // Prva lokacija
                    var dropoffLoc = locations[1]; // Druga lokacija
                    ...
                    var requestedAt = baseDate.AddHours(-1); // Pre 1 sat
                    var startedAt = requestedAt.AddMinutes(5); // Početa pre 55 minuta
                    var distance = 5.5m;
                    var duration = 20;
                    var fare = 12.0m;
                    ...
                    ridesToSeed.Add((
                        RiderId: rider.UserId,
                        DriverId: mirzaBegic.DriverId,
                        VehicleId: mirzaVehicle.VehicleId,
                        PickupLocationId: pickupLoc.LocationId,
                        DropoffLocationId: dropoffLoc.LocationId,
                        RequestedAt: requestedAt,
                        StartedAt: (DateTime?)startedAt,
                        CompletedAt: (DateTime?)null, // Još nije završena
                        Status: "active",
                        FareEstimate: fare,
                        FareFinal: (decimal?)null, // Još nije završena
                        DistanceKm: (decimal?)distance,
                        DurationMin: (int?)duration
                    ));
                }
        ...
        private static async Task UpsertPaymentsAsync(TaxiMoDbContext context, DateTime baseDate)
        {
            ...
            // Other users - fewer payments (about 60-80% of completed rides)
            ...
            foreach (var userRides in otherUsersRides)
            {
                ...
                // Each user pays for 60-80% of their completed rides
                var ridesToPay = userRides
                    .Take((int)(userRides.Count() * (0.6 + (user.UserId % 3) * 0.1)))
                    .ToList();
